...
function kp_block_global_cate($conf) {
	...
	$pages = pages($page, $maxpage, $run->category->format_url($cid, $run->_var['alias'], $run->_cfg, TRUE));
	...
	foreach($list_arr as &$v) {
		...
		if(isset($cate_arr)) {
			...
			$v['cate_url'] = $run->category->format_url($cate_arr[$v['cid']]['cid'], $cate_arr[$v['cid']]['alias'], $run->_cfg);
		}
	}
